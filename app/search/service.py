...
PROPERTY_DEFINITIONS: dict[str, PropertyDefinition] = {
    # Bookmark properties
    "bookmark.post_id": PropertyDefinition(
        "bookmark.post_id", "Bookmark post ID", "b.post_id", "text", TEXT_OPERATORS
    ),
    "bookmark.is_bookmarked": PropertyDefinition(
    ...
    "bookmark.first_seen_at": PropertyDefinition(
        "bookmark.first_seen_at",
        "First seen at",
        "b.first_seen_at",
        "datetime",
        DATE_OPERATORS,
    ),
    "bookmark.last_seen_at": PropertyDefinition(
        "bookmark.last_seen_at",
        "Last seen at",
        "b.last_seen_at",
        "datetime",
        DATE_OPERATORS,
    ),
    "bookmark.inactive_at": PropertyDefinition(
        "bookmark.inactive_at",
        "Inactive at",
        "b.inactive_at",
        "datetime",
        DATE_OPERATORS,
    ),
    "bookmark.last_sync_run_id": PropertyDefinition(
        "bookmark.last_sync_run_id",
        "Last sync run ID",
        "b.last_sync_run_id",
        "number",
        NUMBER_OPERATORS,
    ...
    "post.author_id": PropertyDefinition(
        "post.author_id", "Post author ID", "p.author_id", "text", TEXT_OPERATORS
    ),
    "post.text": PropertyDefinition(
        "post.text", "Post text", "p.text", "text", TEXT_OPERATORS
    ),
    "post.full_text": PropertyDefinition(
        "post.full_text", "Full text", "p.full_text", "text", TEXT_OPERATORS
    ),
    "post.created_at": PropertyDefinition(
        "post.created_at", "Created at", "p.created_at", "datetime", DATE_OPERATORS
    ),
    "post.lang": PropertyDefinition(
        "post.lang", "Language", "p.lang", "text", TEXT_OPERATORS
    ),
    "post.possibly_sensitive": PropertyDefinition(
    ...
    "post.conversation_id": PropertyDefinition(
    ...
    "post.public_metrics_json": PropertyDefinition(
        "post.public_metrics_json",
        "Public metrics JSON",
        "p.public_metrics_json",
        "json",
        TEXT_OPERATORS,
    ),
    "post.entities_json": PropertyDefinition(
        "post.entities_json", "Entities JSON", "p.entities_json", "json", TEXT_OPERATORS
    ),
    "post.attachments_json": PropertyDefinition(
        "post.attachments_json",
        "Attachments JSON",
        "p.attachments_json",
        "json",
        TEXT_OPERATORS,
    ),
    "post.referenced_tweets_json": PropertyDefinition(
        "post.referenced_tweets_json",
        "Referenced tweets JSON",
        "p.referenced_tweets_json",
        "json",
        TEXT_OPERATORS,
    ),
    "post.raw_json": PropertyDefinition(
        "post.raw_json", "Post raw JSON", "p.raw_json", "json", TEXT_OPERATORS
    ),
    "post.updated_at": PropertyDefinition(
        "post.updated_at", "Post updated at", "p.updated_at", "datetime", DATE_OPERATORS
    ),
    # Metrics extracted from post JSON
    "post.metrics.like_count": PropertyDefinition(
        "post.metrics.like_count",
        "Like count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.like_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    "post.metrics.reply_count": PropertyDefinition(
        "post.metrics.reply_count",
        "Reply count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.reply_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    "post.metrics.retweet_count": PropertyDefinition(
        "post.metrics.retweet_count",
        "Retweet count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.retweet_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    "post.metrics.quote_count": PropertyDefinition(
        "post.metrics.quote_count",
        "Quote count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.quote_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    "post.metrics.bookmark_count": PropertyDefinition(
        "post.metrics.bookmark_count",
        "Bookmark count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.bookmark_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    "post.metrics.impression_count": PropertyDefinition(
        "post.metrics.impression_count",
        "Impression count",
        "COALESCE(CAST(json_extract(p.public_metrics_json, '$.impression_count') AS INTEGER), 0)",
        "number",
        NUMBER_OPERATORS,
    ),
    # Author properties
    "author.id": PropertyDefinition(
        "author.id", "Author ID", "u.id", "text", TEXT_OPERATORS
    ),
    "author.username": PropertyDefinition(
        "author.username", "Author username", "u.username", "text", TEXT_OPERATORS
    ),
    "author.name": PropertyDefinition(
        "author.name", "Author name", "u.name", "text", TEXT_OPERATORS
    ),
    "author.description": PropertyDefinition(
        "author.description",
        "Author description",
        "u.description",
        "text",
    ...
    "author.verified": PropertyDefinition(
        "author.verified", "Author verified", "u.verified", "boolean", BOOL_OPERATORS
    ),
    "author.profile_image_url": PropertyDefinition(
    ...
    "author.raw_json": PropertyDefinition(
        "author.raw_json", "Author raw JSON", "u.raw_json", "json", TEXT_OPERATORS
    ),
    "author.updated_at": PropertyDefinition(
        "author.updated_at",
        "Author updated at",
        "u.updated_at",
        "datetime",
    ...
    # Derived properties
    "derived.has_media": PropertyDefinition(
        "derived.has_media",
        "Has media",
        "EXISTS (SELECT 1 FROM media m WHERE m.post_id = p.id)",
        "boolean",
        BOOL_OPERATORS,
    ),
    "derived.media_count": PropertyDefinition(
        "derived.media_count",
        "Media count",
    ...
    "derived.hashtags": PropertyDefinition(
        "derived.hashtags",
        "Hashtags",
        "COALESCE(post_search.hashtags, '')",
        "text",
        TEXT_OPERATORS,
    ),
    "derived.mentions": PropertyDefinition(
        "derived.mentions",
        "Mentions",
        "COALESCE(post_search.mentions, '')",
        "text",
        TEXT_OPERATORS,
    ),
    "derived.urls": PropertyDefinition(
        "derived.urls", "URLs", "COALESCE(post_search.urls, '')", "text", TEXT_OPERATORS
    ),
}
...
class SearchService:
    ...
    def reindex_post(self, post_id: str) -> None:
        with self.db.connect() as connection:
            ...
            if not row:
                return
            entities = (
                json.loads(row["entities_json"] or "{}") if row["entities_json"] else {}
            )
            hashtags = " ".join(
                item.get("tag", "")
            ...
            urls = " ".join(
                (item.get("expanded_url") or item.get("url") or "")
    ...
    def search(
        ...
        if sort_by:
            order_by = self._compile_sort(
                sort_by, sort_order, query_present=bool(query.strip())
            )
    ...
    def get_post(self, post_id: str) -> dict[str, Any] | None:
        ...
        entities = (
            json.loads(post["entities_json"] or "{}") if post["entities_json"] else {}
        )
        public_metrics = (
            json.loads(post["public_metrics_json"] or "{}")
            if post["public_metrics_json"]
            else {}
        )
        return {
            "id": post["id"],
            ...
            if post["is_bookmarked"] is not None
            else False,
    ...
    def _compile_sort(self, sort_by: str, sort_order: str, query_present: bool) -> str:
        ...
        if not definition or not definition.sortable:
            return (
                "score ASC, b.last_seen_at DESC, p.created_at DESC"
                if query_present
                else "b.last_seen_at DESC, p.created_at DESC"
            )
